...
export default function OrderConfirm() {
    ...
    useEffect(() => {
        ...
        if (selectedAdress) {
            ...
            document.getElementById(selectedAdress.id).classList.add("selected-adress");
        }
    }, [selectedAdress])
    ...
    const order = () => {
        ...
        if (selectedAdress) {
            if (userData.cvv && userData.expiration_CB && userData.number_CB) {
                const array = [];
                cart.forEach((e) => {
                    array.push({ id: e.id, quantity: e.selectedQuantity, price: e.price });
                });
                const data = {
                    user_id,
        ...
        return (
            <div>
                ...
                <div className="order-confirm-adresses">
                    {
                        userData.adresses.length > 0
                            ? userData.adresses.map((e) => {
                                return (
                                    <div id={e.id} key={e.id} onClick={() => setSelectedAdress(e)} className="order-adress">
                            ...
                            : <div>
                                <form className="vertical-form" ref={newAdressForm} onSubmit={addAdress} encType="multipart/form-data">
                                    ...
                                    <input required type="submit" value="Utiliser cette adresse"></input>
                                </form>
                            </div>
                    }
                </div>
                <div>
                    ...
                    {userData.cvv
                        ? <p>{userData.number_CB} {userData.expiration_CB} {userData.cvv}</p>
                        : <form className="vertical-form" ref={CBForm} onSubmit={submitCBData} encType="multipart/form-data">
                            <h3>Nouvelle moyen de paiement</h3>
                            <input type="hidden" name="firstname" placeholder="Prénom" defaultValue={userData.firstname}></input>
                            <input type="hidden" name="lastname" placeholder="Nom de famille" defaultValue={userData.lastname}></input>
                            <input type="hidden" name="email" placeholder="Email" defaultValue={userData.email}></input>
                            <input type="hidden" name="telephone" placeholder="Téléphone" defaultValue={userData.telephone}></input>
                            <input type="text" name="number_CB" placeholder="Numéro de carte bancaire" minLength={16} maxLength={16} defaultValue={userData.number_CB}></input>
                            <input type="text" name="cvv" placeholder="CVV" minLength={3} maxLength={3} defaultValue={userData.cvv}></input>
                            <input type="text" name="expiration_CB" placeholder="Date d'expiration" minLength={5} maxLength={5} defaultValue={userData.expiration_CB}></input>
